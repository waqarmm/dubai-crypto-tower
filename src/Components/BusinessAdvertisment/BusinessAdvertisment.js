...
const BusinessAdvertisment = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-full sm:w-3/5 h-3/4	 lg:w-2/5   2xl:h-2/3  bg-lightBlack mt-10  flex justify-evenly items-center flex-col px-1 sm:px-10 ${className}`}
        >
          {children}
          <Paragraph
          ...
          <div className="text-center">
            <AnchorTag
              to={link}
              className="border-white text-white px-6 lg:px-2  xl:px-8 hover:text-black hover:bg-white"
              title={buttonText}
            />
          </div>
        </div>
